# load libraries
library(pacman)
p_load(tidymodels, tidyverse, doParallel, tictoc, nnet, brulee)
...
mlp_model <- mlp(
  ...
  penalty = tune(),
  learn_rate = .1) %>%
  set_engine("brulee", optimizer = "SGD", 
             momentum = tune(), mixture = tune())

## mlp parameters
mlp_params <- extract_parameter_set_dials(mlp_model) %>% 
  update(hidden_units = hidden_units(c(1, 7)),
         penalty = penalty(c(-2, 2))) 
  
mlp_grid <- grid_regular(mlp_params, levels = 3)
